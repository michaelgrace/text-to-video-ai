...
from app.core.pexels_query_generator import build_pexels_query  # <-- Add this import

PEXELS_API_KEY = os.environ.get('PEXELS_API_KEY')

def search_videos(theme, topic, orientation_landscape=True):
    # Use build_pexels_query to construct the query string
    query_string = build_pexels_query(theme, topic)
...
def getBestVideo(theme, topic, orientation_landscape=True, used_vids=[]):
    vids = search_videos(theme, topic, orientation_landscape)
        ...
        for video_file in video['video_files']:
            if orientation_landscape:
                ...
                if video_file['width'] == 1080 and video_file['height'] == 1920:
                    if not (video_file['link'].split('.hd')[0] in used_vids):
                        return video_file['link']
    print("NO LINKS found for this round of search with theme and topic:", theme, topic)
    return None


def generate_video_url(timed_video_searches, video_server, theme):
    timed_video_urls = []
    if video_server == "pexel":
        ...
        for (t1, t2), search_terms in timed_video_searches:
            ...
            for topic in search_terms:
                # Pass theme and topic to getBestVideo
                url = getBestVideo(theme, topic, orientation_landscape=True, used_vids=used_links)
                if url:
